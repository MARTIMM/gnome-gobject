...
#`{{
    ...
    my $v = self.get-native-gboxed;
    my $value = %options<value>;

    given $type {
      when G_TYPE_BOOLEAN { $v.set-boolean($value); }
      when G_TYPE_CHAR { $v.set-schar($value); }
      when G_TYPE_UCHAR { $v.set-uchar($value); }
      when G_TYPE_INT { $v.set-int($value); }
      when G_TYPE_UINT { $v.set-uint($value); }
      when G_TYPE_LONG { $v.set-long($value); }
      when G_TYPE_ULONG { $v.set-ulong($value); }
      when G_TYPE_INT64 { $v.set-int64($value); }
      when G_TYPE_UINT64 { $v.set-uint64($value); }
      when G_TYPE_FLOAT { $v.set-float($value); }
      when G_TYPE_DOUBLE { $v.set-double($value); }
      when G_TYPE_STRING { $v.set-string($value); }
      when G_TYPE_ENUM { $v.set-enum($value); }
      when G_TYPE_FLAGS { $v.set-flags($value); }

      when G_TYPE_OBJECT {note "Type object for $value not yet available";}# { $v.set-($value); }
      when G_TYPE_POINTER {note "Type pointer for $value not yet available";}# { $v.set-($value); }
      when G_TYPE_BOXED {note "Type boxed for $value not yet available";}# { $v.set-($value); }
      when G_TYPE_PARAM {note "Type param for $value not yet available";}# { $v.set-($value); }
      when G_TYPE_VARIANT {note "Type variant for $value not yet available";}# { $v.set-($value); }
    }
  }
}}
  elsif %options.keys.elems {
...
# signed char
sub g_value_set_schar ( N-GValue $value, int8 $v_schar )
  is native(&gobject-lib)
  { * }
...
# unsigned char
sub g_value_set_uchar ( N-GValue $value, uint8 $v_schar )
  is native(&gobject-lib)
  { * }
